...
export async function POST(req) {
  ...
  if (!request.userId || !request.prevPassword || !request.newPassword)
  ...
  const [[pass]] = await db.query(
    "SELECT pass_hash FROM users WHERE user_id = ?",
    [request.userId],
  );
  ...
  const query = `UPDATE users SET pass_hash = ${hashedPassword} WHERE user_id = ${request.userId};`;
