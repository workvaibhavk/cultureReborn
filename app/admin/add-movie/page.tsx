...
export default function Page() {
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-gray-100">
      ...
      {query != "" && data && (
        <div className="shadow-lg bg-green-200 p-3 rounded-xl absolute max-w-74 left-10 top-12">
          {query}
          <button
            type="button"
            onClick={() => setQuery("")}
      ...
      <div className="w-full max-w-sm rounded-xl bg-white p-8 shadow-lg">
        ...
        {error && (
          <p className="bg-red-400 text-sm text-white py-2 px-3 font-bold rounded-md text-center mx-auto">
            {error}
          </p>
        )}
        ...
        <form className="space-y-4 mt-6" onSubmit={handleSubmit}>
          ...
          {posterUrl && (
            <button
              type="button"
              onClick={() => previewwPoster()}
              className="bg-green-200 py-2 px-4 mx-auto rounded-md"
            >
              Preview Poster
            </button>
          )}
            ...
            <div className="mt-3">
              {cast &&
                cast.map((item) => (
                  <div
                    key={item}
                    onClick={() => minusToggleCast(item)}
                    className="bg-[#e6cefa] py-[8px] px-[16px] border-[#ddd] rounded-2xl mr-[5px] cursor-pointer inline-flex items-center gap-[6px] "
                  >
                    {item}
                  </div>
                ))}
          ...
          <div>
            ...
            <span className="border-2 border-black py-[12px] bg-black rounded-xl">
              <button
                type="button"
                onClick={() => setAgeRating("U")}
                className={`${ageRating == "U" ? " bg-blue-600 " : "text-blue-600 bg-black"} py-2 px-4 rounded-xl font-bold text-lg`}
              >
                U
              </button>
              <button
                type="button"
                onClick={() => setAgeRating("UA")}
                className={`${ageRating == "UA" ? " bg-blue-600 " : "text-blue-600 bg-black"} py-2 px-4 rounded-xl font-bold text-lg`}
              >
                UA
              </button>
              <button
                ...
                className={`${ageRating == "A" ? " bg-blue-600 " : "text-blue-600 bg-black"} py-2 px-4 rounded-xl font-bold text-lg`}
              >
                A
              </button>
            </span>
